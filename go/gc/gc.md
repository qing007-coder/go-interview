...
- **清除阶段**：遍历堆中所有的对象，清除那些没有被标记为可达的对象，将它们的内存归还给系统。（有点像找图的最大连通分量，剩下来的就是不用的变量，会被回收）
...
#### **概念**
- **压缩** 是一种减少内存碎片的技术。在某些垃圾回收器中，清除阶段不仅回收了未使用的对象，还将存活对象移动到内存的连续区域，避免内存碎片化。（这个还挺像os中的内存紧凑算法）
