...
- P的数量由`GOMAXPROCS`设置决定，它决定了并行执行的最大线程数。(runtime.GOMAXPROCS(cpu核心的数量))
...
- **工作窃取机制**：如果一个P的本地队列为空，而另一个P的本地队列中有多个Goroutine，前者可以从后者中窃取任务，从而保持系统的高效利用率。（可以理解成负载均衡）
- **阻塞与调度**：当M执行的Goroutine阻塞（例如I/O操作）时，M会释放当前的P并等待P重新分配任务，从而避免资源浪费。（有点cpu的机制，io中断之类的）
