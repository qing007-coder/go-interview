...
| 特性   | 悲观锁                   | 乐观锁（CAS）                       |
| ---- |-----------------------|--------------------------------|
| 核心思想 | 先假设会冲突 → 抢锁 → 访问 → 解锁 | 先假设不会冲突 → 读值 → 更新 → CAS 成功才算完成 |
| 阻塞情况 | 其他线程等待锁 → 阻塞（内核态）     | CAS 失败 → 自旋重试 → 不阻塞其他线程（用户态）   |
| 适用场景 | 锁冲突频繁、临界区复杂           | 锁冲突少、操作简单（比如计数器）               |
| 系统开销 | 上下文切换、内核调度成本高         | 原子操作在 CPU 指令级别完成，开销低           |
| 并发性能 | 高竞争下容易严重阻塞            | 高并发下性能通常更好                     |
